...
const MENU_PATHS = [
  "/menu", "/carta", "/menus", "/our-menu", "/food-menu",
  "/menu/", "/carta/", "/menus/", "/gastronomy", "/gastronomia",
  "/ementa", "/restaurante", "/restaurant/menu", "/food", "/dining",
  "/comer", "/what-we-serve", "/our-food",
];
...
async function scrapeMenuLink(website: string): Promise<string | null> {
  try {
    ...
    const menuKeywords = /\b(menu|carta|menus|ementa|food|dining|gastronom|comer)\b/i;
...
async function nominatimWebsite(
  ...
  try {
    ...
    if (lat && lng) {
      const D = 0.3;
      params.set("viewbox", `${parseFloat(lng) - D},${parseFloat(lat) + D},${parseFloat(lng) + D},${parseFloat(lat) - D}`);
...
function mapsMenuUrl(name: string, dest: string, lat: string | null, lng: string | null): string {
  const q = encodeURIComponent(`${name} ${dest} carta menu`);
  if (lat && lng) return `https://www.google.com/maps/search/${q}/@${lat},${lng},17z`;
