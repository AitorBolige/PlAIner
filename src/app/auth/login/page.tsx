...
function LoginPageInner() {
  ...
  return (
    <div className="pl-fadein" style={{ position: 'relative', height: '100%', overflow: 'hidden', background: 'var(--surface)' }}>
      {/* Hero — compact */}
      <div style={{
        height: '38%',
        ...
        <div style={{ position: 'absolute', inset: 0, pointerEvents: 'none' }}>
          <div className="pl-float-1" style={{ position: 'absolute', top: -60, right: -40, width: 200, height: 200, borderRadius: '50%', background: 'rgba(255,255,255,0.07)' }} />
          <div className="pl-float-2" style={{ position: 'absolute', bottom: -30, left: -60, width: 160, height: 160, borderRadius: '50%', background: 'rgba(255,255,255,0.05)' }} />
        </div>
        <div style={{ position: 'relative', height: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', padding: '44px 24px 24px' }}>
          <div>
            <span style={{ fontFamily: 'var(--font-display)', fontSize: 20, fontWeight: 800, letterSpacing: '-0.03em', color: '#fff' }}>
              PL<span style={{ color: 'rgba(255,255,255,0.55)' }}>AI</span>ner
            </span>
            <p style={{ fontSize: 11.5, color: 'rgba(255,255,255,0.55)', marginTop: 2 }}>planifica sense fricció</p>
          </div>
          <div>
            <h1 style={{ fontFamily: 'var(--font-display)', fontSize: 28, fontWeight: 800, color: '#fff', letterSpacing: '-0.03em', lineHeight: 1.1, marginBottom: 4 }}>
              El món t&apos;espera.
            </h1>
            <p style={{ fontSize: 13, color: 'rgba(255,255,255,0.60)' }}>El teu pròxim viatge, a 3 minuts.</p>
          </div>
        </div>
      </div>
      ...
      {/* Card */}
      <div style={{
        position: 'absolute', top: 'calc(38% - 22px)', left: 0, right: 0, bottom: 0,
        ...
        padding: '22px 22px 16px', zIndex: 10, display: 'flex', flexDirection: 'column',
      }}>
        {registered && (
          <div style={{ marginBottom: 12, padding: '10px 14px', background: 'var(--green-subtle)', border: '1px solid var(--green-glow)', borderRadius: 12, fontSize: 13, color: 'var(--green-deep)', fontWeight: 500 }}>
            ✓ Compte creat. Inicia sessió per continuar.
          </div>
        )}
        <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 20, fontWeight: 700, color: 'var(--text)', letterSpacing: '-0.02em', marginBottom: 2 }}>
          Benvingut de nou
        </h2>
        ...
        <form onSubmit={onSubmit} style={{ display: 'flex', flexDirection: 'column', flex: 1 }}>
          <div style={{ marginBottom: 10 }}>
            <Field icon={<Mail size={16} />} focus={emailFocus}>
              <input type="email" value={email} onChange={(e) => setEmail(e.target.value)}
                placeholder="joana@plainer.app" autoComplete="email" required
          ...
          <div style={{ marginBottom: 6 }}>
            <Field icon={<Lock size={16} />} focus={passFocus}
              right={
          ...
          <div style={{ textAlign: 'right', marginBottom: 14 }}>
            <Link href="/auth/forgot-password" style={{ fontSize: 12.5, fontWeight: 500, color: 'var(--green)', textDecoration: 'none' }}>
              He oblidat la contrasenya
            </Link>
          </div>

          {error && <p style={{ fontSize: 13, color: 'rgba(240,90,53,0.95)', marginBottom: 8 }}>{error}</p>}

          <button type="submit" disabled={loading} className="pl-tap"
            style={{ width: '100%', height: 50, background: 'var(--green)', color: '#fff', border: 'none', borderRadius: 'var(--r-pill)', fontSize: 15, fontWeight: 700, fontFamily: 'var(--font-display)', letterSpacing: '-0.01em', boxShadow: 'var(--shadow-cta)', cursor: 'pointer', marginBottom: 14, opacity: loading ? 0.8 : 1 }}>
            {loading ? "Carregant…" : "Iniciar sessió"}
          </button>

          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
            <div style={{ flex: 1, height: 1, background: 'var(--border)' }} />
            <span style={{ fontSize: 11.5, color: 'var(--text-faint)', whiteSpace: 'nowrap' }}>o continua amb</span>
            <div style={{ flex: 1, height: 1, background: 'var(--border)' }} />
          </div>

          <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 16 }}>
            <button type="button" onClick={() => signIn("google", { callbackUrl: "/plainer-mvp.html" })} className="pl-tap"
            ...
            <button type="button" onClick={() => signIn("facebook", { callbackUrl: "/plainer-mvp.html" })} className="pl-tap"
          ...
          <p style={{ textAlign: 'center', fontSize: 13.5, color: 'var(--text-muted)', marginTop: 'auto' }}>
            No tens compte?{" "}
            <Link href="/auth/register" style={{ color: 'var(--green)', fontWeight: 600, textDecoration: 'none' }}>Crear-la →</Link>
          </p>
        </form>
      </div>
    </div>
