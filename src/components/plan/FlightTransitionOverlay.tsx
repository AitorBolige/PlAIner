...
function greatCircleArc(
  ...
  for (let i = 0; i <= steps; i++) {
    ...
    if (δ < 0.0001) {
      pts.push([lng1 + (lng2 - lng1) * f, lat1 + (lat2 - lat1) * f]);
      continue;
    }
...
export function FlightTransitionOverlay({
  ...
  const origin: [number, number] = AIRPORT_COORDS[originCode] ?? [
    -3.5673, 40.4719,
  ];
  ...
  React.useEffect(() => {
    ...
    map.on("style.load", () => {
      ...
      new mapboxgl.Marker({ element: mkOrigin, anchor: "center" })
        .setLngLat(origin)
        .addTo(map);

      // Dest marker (revealed on arrival)
      const mkDest = document.createElement("div");
      mkDest.style.cssText =
      ...
      new mapboxgl.Marker({ element: mkDest, anchor: "center" })
        .setLngLat(dest)
        .addTo(map);
          ...
          >
            ...
            <div
              style={{
                ...
                pointerEvents: "none",
                background:
                  "linear-gradient(to bottom, #f4f1ec 0%, transparent 22%, transparent 72%, #f4f1ec 100%)",
              }}
            />
            ...
            >
              ...
              >
                ...
                <span
                  style={{ fontSize: 14, fontWeight: 700, color: "#0D9E7A" }}
                >
                  {flightTime}
                </span>
                <span
                  style={{ fontSize: 12, color: "#7a7570", fontWeight: 500 }}
                >
                  vol estimat
                </span>
              </motion.div>
              ...
              >
                <span
                  style={{
                    ...
                    animation:
                      status === "En vol"
                        ? "pdot 1.2s ease-in-out infinite"
                        : "none",
                  }}
                />
                <span
