...
type DayWithDayId = DayDTO & {
  /** parent day id for each activity (kept in a Map for fast lookup). */
};
  ...
  return (
    ...
    >
      <div className="flex items-start gap-3">
        <button
          type="button"
          aria-label="Arrastrar activitat"
          {...attributes}
          {...listeners}
...
export function EditableItinerary({
  ...
  function handleDragOver(e: DragOverEvent) {
    ...
    // overId can be an activity id (sortable item) or a day id (container)
  ...
  async function onSave() {
    ...
    try {
      ...
      toast.success("Itinerari guardat correctament.");
    } catch (e) {
      toast.error("No hem pogut guardar els canvis.");
    } finally {
      setSaving(false);
    }
  }
  ...
  return (
    <div className="grid gap-3">
      <div className="flex items-center justify-between gap-3">
        <div className="text-xs text-muted">
          Arrossega les activitats per reordenar-les o moure-les a un altre dia.
        </div>
        {dirty ? (
          <Button
            type="button"
          ...
          >
            {saving ? (
              <span className="inline-flex items-center gap-1">
                <Loader2 size={14} className="animate-spin" /> Guardant…
              </span>
            ) : (
              <span className="inline-flex items-center gap-1">
